...
GA4 measurement ID `G-7LXYJ200S2` is installed directly through the supplied Google tag, sharing dataLayer with GTM-KBSMCVBN. The published GTM container did not contain this GA4 ID when checked on September 11, 2026. Do not add a second configuration for the same measurement ID in GTM while this direct installation is active. Confirm received data in GA4 Realtime; account reports and key-event configuration require the Analytics account owner. The form's `kmch_lead_submitted` custom dataLayer event is retained for GTM. GA4 also receives `generate_lead` directly after a confirmed submission; do not configure a second GA4 generate_lead tag for the GTM event. See `TRACKING-SETUP.md` for all tracked actions and the remaining account setup.
